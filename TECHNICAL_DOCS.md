...
| `GROQ_API_KEY` | AI concierge (free tier) | `.env.local` + Vercel |
| `GROQ_API_KEY_2` | Optional second Groq account — a second per-minute allowance | Vercel |

The model order lives in `src/lib/ai-provider.ts` and is shared by the site
concierge and the Telegram bot. Either key alone is enough; an account that
answers 401/403/429/5xx hands the question to the next entry, so a rotated or
rate-limited key degrades the answer rather than breaking the assistants.

> **Windows note:** a real API key in the user environment beats `.env.local` —
> Next.js will not override a variable the OS already set. A stale one is how
> every question once returned 502 locally while healthy accounts sat idle.
> Check with `[Environment]::GetEnvironmentVariable("<NAME>","User")`.
