...
export const topchanPricing = {
   ...
   *
   * У бассейна СВОИ часы, и он закрывается позже — не сводить их в одну
   * константу: гость, приехавший на топчан к семи вечера, приедет зря.
   */
  hours: "10:00–18:00",
...
export const tubingPricing = {
  ...
  allSeason: true,
  /**
   * Часы горки (оператор, голосом 17.08.2026): «утром тюбинг работает с 10»,
   * «после восьми катание запрещается».
   *
   * Своё окно, третье по счёту: топчан закрывается в 18:00, бассейн — в 20:00
   * для посетителей и в 20:00 для проживающих, горка тоже до 20:00, но
   * открывается в 10:00.
   */
  hours: "10:00–20:00",
  /**
   * Сколько спусков включено проживающему — ОДИН НА ГОСТЯ (оператор, голосом
   * 17.08.2026: «для жильцов тюбинг по одному катанию бесплатно»).
   *
   * Прежняя формулировка «2 спуска глэмпингу, 4 шале» говорила то же самое
   * другими словами: это ровно базовая вместимость домиков, помноженная на
   * один спуск. Правило записано на гостя, потому что так его назвал оператор
   * и потому что при доплате за дополнительное место вопрос «а ему спуск
   * положен» решается сам собой.
   */
  includedRidesPerGuest: 1,
  /**
   * Оговорка оператора, сказанная дважды: бесплатный спуск действует ТОЛЬКО
   * при работающей горке, и кататься без инструктора нельзя ни в какое время.
   * Первое — про погоду и техработы, второе — про безопасность.
   */
  onlyWhenOperating: true,
...
export const poolPricing = {
  /**
   * Часы ПОСЕТИТЕЛЯ — того, кто покупает дневной билет. Именно это время
   * печатают форма заявки, тариф и ответы гостю: страница бассейна продаёт
   * дневной вход.
   *
   * За сутки цифра менялась дважды: 16 августа оператор назвал 10:00–22:00,
   * 17-го уточнил — у проживающих и у посетителей окна РАЗНЫЕ, и закрывается
   * бассейн в 20:00. Одной строкой это не описывается, поэтому их две.
   */
  hours: "10:00–20:00",
  /** Проживающим в шале и глэмпинге бассейн открывается на два часа раньше. */
  hoursForStayingGuests: "08:00–20:00",
  ...
  /**
   * Бассейн вокруг проживания — дополнительные условия оператора от 17.08.2026.
   *
   * ДО ЗАЕЗДА: гость с подтверждённой бронью купается бесплатно, ожидая
   * заселения. Это не «ранний заезд» — домик не выдаётся и не оплачивается.
   *
   * ПОСЛЕ ВЫЕЗДА: проживание кончилось, и бассейн больше не входит в его
   * стоимость — фиксированные ${money} за день, независимо от дня недели и
   * от того, сколько стоил дневной билет. Именно поэтому это отдельное число,
   * а не ссылка на adult.weekday: полоса Пн–Чт / Пт–Вс здесь не действует.
   */
  beforeCheckInFree: true,
  afterCheckOut: 100_000,
...
export const stayRules = {
  ...
  checkOut: "12:00",
  /**
   * Сколько есть на предоплату после оформления брони (оператор, 2026-08-17).
   *
   * ВНИМАНИЕ: подписанная оферта, пп. 1.2 и 3.6, говорит про 24 часа — этот
   * текст воспроизводится дословно из .docx и здесь не правится. Пока оператор
   * не выпустит новую редакцию, сайт и документ расходятся; выбрано указание
   * оператора, потому что по нему работает ресепшен.
   */
  prepayWithin: { ru: "1 часа", uz: "1 soat", en: "1 hour" },
  /**
   * Завтрак. Стоял цифрами в семи местах — меню, перки домиков, FAQ, оба
   * брифинга и шторка бронирования: ровно та россыпь, из-за которой время
   * расходится при первой же правке.
   */
 ...
 */
/**
 * Возвратный депозит за домик — оферта, п. 5.1.6.
 *
 * Не предоплата: возвращается целиком при выезде, если ничего не повреждено.
 * Оказался единственной суммой, которую гость видит на странице, а оператор не
 * мог изменить нигде — она была вписана прозой в брифинги ИИ.
 */
export const depositPricing = { perCabin: 500_000 } as const;

/**
 * Туристский сбор.
 *
 * Уточнение оператора (2026-08-11): сбор взимается **только с иностранных
 * граждан и лиц без гражданства** при заселении в глэмпинг и шале. С граждан и
 * резидентов Узбекистана он НЕ берётся — до этого сайт называл им 1 692 сум,
 * то есть выставлял платёж, которого нет.
 *
 * Ставка — «действующая на дату заезда»: её устанавливает государство и может
 * поменять без нашего участия. Поэтому цифра здесь и в админке — справочная, а
 * тексты обязаны говорить «в размере, действующем на дату заезда», и никогда —
 * «ровно столько», иначе при первом же изменении закона сайт начнёт врать.
 */
export const touristTax = {
  /**
   * Легаси. Ставки для резидентов не существует; поле оставлено, чтобы не
   * ломать форму данных и сохранённый патч оператора, но ни один текст его
   * больше не печатает.
   */
  resident: 0,
  /** Иностранные граждане и лица без гражданства, за ночь с человека. */
  nonResident: 61_800,
} as const;
