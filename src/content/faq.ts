...
import { poolPricing, stayRules, topchanPricing, tubingPricing } from "./pricing";
...
export function faqItems(live: LivePricing = resolvePricing()): FaqItem[] {
  ...
  },
  {
    ...
    answer: {
      ru: `Да, на день можно приехать тремя способами. Топчан — ${P.topchanWeekday} сум в будни (Пн–Чт) и 300 000 в выходные (Пт–Вс) за топчан целиком, до 8 гостей. Бассейн — 100 000 и ${P.poolAdultWeekend} сум с человека, дети 5–15 вдвое дешевле, до 5 лет бесплатно. Тюбинг-горка — ${P.ride2} сум за 2 спуска и 100 000 за 4, цена одна всю неделю. Топчан работает ${P.topchanHours}, бассейн — ${P.poolHours}, горка — ${P.tubingHours}: время разное, планируйте приезд по нужному. Кататься на горке можно только с инструктором. Вход на территорию бесплатный; парковка платная только для тюбинга — 50 000 и ${P.poolAdultWeekday} сум за автомобиль. У каждого формата своя форма заявки на сайте — администратор перезвонит и подтвердит. Заявку лучше оставить заранее: топчан и место у бассейна закрепляются только после подтверждения, и приехавшим без заявки свободное место не гарантировано.`,
      uz: `Ha, bir kunga uch xil kelish mumkin. Topchan — ish kunlari (Du–Pay) ${P.topchanWeekday} so'm, dam olish kunlari (Ju–Yak) ${P.topchanWeekend} so'm butun topchan uchun, 8 kishigacha. Basseyn — bir kishidan 100 000 va ${P.poolAdultWeekend} so'm, 5–15 yosh ikki barobar arzon, 5 yoshgacha bepul. Tubing gorkasi — 2 marta uchish ${P.ride2} so'm, 4 marta ${P.poolAdultWeekday} so'm, narx butun hafta bir xil. Topchan ${P.topchanHours}, basseyn ${P.poolHours}, gorka ${P.tubingHours} ishlaydi — vaqtlari har xil. Gorkada faqat instruktor bilan uchish mumkin. Hududga kirish bepul; parkovka faqat tubing uchun to'lanadi — avtomobil uchun 50 000 va ${P.poolAdultWeekday} so'm. Har bir formatning saytda o'z arizasi bor — administrator qo'ng'iroq qilib tasdiqlaydi. Arizani oldindan qoldirgan ma'qul: topchan va basseyndagi joy faqat tasdiqlangandan keyin biriktiriladi, arizasiz kelganlarga bo'sh joy kafolatlanmaydi.`,
      en: `Yes, in three ways. A topchan costs ${P.topchanWeekday} UZS Mon–Thu and 300 000 Fri–Sun for the whole platform, seating up to 8. The pool is 100 000 and ${P.poolAdultWeekend} UZS per person, half price for ages 5–15 and free under five. The tubing hill is ${P.ride2} UZS for 2 rides and 100 000 for 4, at one price all week. The topchan runs ${P.topchanHours}, the pool ${P.poolHours} and the hill ${P.tubingHours} — different windows, so plan around the one you want. Riding is only allowed with an instructor present. Entry to the grounds is free; parking is charged for tubing only, at 50 000 and ${P.poolAdultWeekday} UZS per car. Each has its own request form on the site, and the administrator calls back to confirm. Send it ahead: a topchan and a place at the pool are held only once confirmed, and arriving without a request does not guarantee a free one.`,
    },
  },
  ];
}
