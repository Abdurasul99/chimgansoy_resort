...
describe("тюбинг — совместный спуск запрещён (оператор, 18.08.2026)", () => {
  ...
  it("возрастной порог остался: младше 5 лет не допускают", () => {
    ...
    expect(tubingDoc).toContain("Дети младше 5 лет");
    expect(tubingDoc).toContain("110 см");
  });

  it("ИИ знает и про запрет, и про порог — иначе пообещает «поедете вместе»", () => {
    for (const text of [venueFacts(), venueCore()]) {
      expect(text).toMatch(/совместный спуск.{0,40}запрещ/i);
      expect(text).toMatch(/младше 5 лет/i);
      expect(text).toMatch(/110 см/);
    }
  });
});

describe("возраст ребёнка при заселении (оператор, 26.08.2026)", () => {
  it("платная полоса начинается с 5 лет, а не с 4", () => {
    // В присланном .docx две половины пункта пересекались: «до 5 лет
    // бесплатно» и тут же «от 4 до 12 лет — по Прейскуранту». Четырёхлетний
    // попадал в обе, и спор на ресепшене решался в пользу настойчивого.
    // legalPolicies уже отдаёт текст с применёнными поправками — это то, что
    // читает гость на странице, а не то, что осталось в исходном .docx.
    // Поправки применяет policies.ts — legalPolicies отдаёт исходный текст.
    const body = JSON.stringify(policies.find((p) => p.slug === "public-offer"));
    expect(body).toContain("от 5 до 12 лет определяется согласно утверждённому Прейскуранту");
    expect(body).not.toContain("от 4 до 12 лет");
  });

  it("прайс сайта говорит то же самое", () => {
...
describe("акции сентября 2026", () => {
  ...
  it("связь акций объяснена на обеих карточках, а не в одной", () => {
    // Официальные условия оператора (пост от 08.09.2026): «Не суммируется с
    // тарифом „Всё включено“». Сайт какое-то время утверждал обратное — что
    // питание в «2+1» входит и платить за него не надо. Это обещание еды,
    // которой в акции нет: гость приезжает за полным пансионом, а отель его
    // не закладывал.
    //
  ...
  it("ИИ знает про акции — иначе пообещает скидку, которой нет", () => {
    // Дата явно: брифинг зависит от сроков акций, и на реальных часах тест
    // начал бы падать сам собой, когда кончится последний заезд.
    const text = venueFacts(undefined, "2026-09-24");
    expect(text).toContain("«2+1»");
    expect(text).toContain("Всё включено");
    // По официальным условиям питание в «2+1» НЕ входит. Если бот скажет
    // обратное, гость приедет за бесплатным пансионом — и будет прав, потому
    // что так ему ответил сайт.
    expect(text).toContain("НЕ суммируется");
    expect(text).not.toContain("ВХОДИТ в акцию");
  });

  it("ИИ знает срок «2+1» и последний день заезда", () => {
    const text = venueFacts(undefined, "2026-09-24");
    expect(text).toContain("до 30 сентября");
    expect(text).toContain("последний заезд — 28 сентября");
  });

  it("«2+1» уходит из брифинга вместе с последним заездом, «Всё включено» — со своим сроком", () => {
    // 29.09: заехать по «2+1» уже нельзя, а «Всё включено» ещё действует.
    ...
    expect(october).not.toContain("«Всё включено» — трёхразовое питание");
  });
});
...
describe("площади домиков — с рабочего чертежа оператора", () => {
  it("шале: 91 м² дом и 42 м² терраса — цифры оператора", () => {
    /*
     * Оператор назвал 91 и 42; на чертеже сумма комнат даёт 79,8, а терраса
     * с навесом — около 35. Разница не ошибка: 91 и 42 габаритные, со стенами
     * и перекрытием навеса. Публикуем то, что назвал оператор, — но держим обе
     * величины рядом, чтобы следующий раз никто не «поправил» 91 на 79,8,
     * посчитав комнаты заново.
     */
    const комнаты = 38.4 + 16.0 + 16.0 + 4.7 + 4.7;
    expect(комнаты).toBeCloseTo(79.8, 1);

    const chalet = rooms.find((r) => r.slug === "cottage");
    expect(chalet?.size.ru).toContain("91 м²");
    expect(chalet?.size.ru).toContain("42 м²");
  });

  it("глэмпинг: 32 м² дом и 15 м² терраса", () => {
    // Прежде стояло 28 м² — занижено: внутренний контур на чертеже 4840 × 6660.
    const glamping = rooms.find((r) => r.slug === "glamping");
    expect(glamping?.size.ru).toContain("32 м²");
    expect(glamping?.size.ru).toContain("15 м²");
    expect(JSON.stringify(rooms)).not.toContain("28 м²");
  });
