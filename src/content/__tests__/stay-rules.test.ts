...
describe("акции сентября 2026", () => {
  ...
  it("связь акций объяснена на обеих карточках, а не в одной", () => {
    // С 14.09.2026 питание «Всё включено» входит в «2+1», поэтому прежний
    // запрет «не суммируется» стал противоречием: платить отдельно нечего.
    // Но связь между акциями гость должен видеть на той карточке, которую
    // открыл, — иначе выяснять её придётся на ресепшене.
    ...
    expect(JSON.stringify(promotions)).not.toContain("Не суммируется");
  });
  ...
  it("ИИ знает про акции — иначе пообещает скидку, которой нет", () => {
    ...
    // С 14.09.2026 питание входит в акцию, а не исключает её. Бот должен
    // знать именно это: прежний запрет «НЕ суммируется» он бы пересказал
    // гостю как «за еду платите отдельно».
    expect(text).toContain("ВХОДИТ в акцию");
    expect(text).not.toContain("НЕ суммируется");
  });
});
