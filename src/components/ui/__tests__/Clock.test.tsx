...
describe("подсветка времени", () => {
  ...
  it("не принимает за время двоеточие внутри длинного числа", () => {
    // Номер стандарта из сертификата: «958:2026», а не «58:20». Подсветка
    // выхватывала середину, и номер документа на странице читался как опечатка.
    render(<Clock text="Соответствует стандарту O'z MSt 958:2026" />);
    // queryAllByText, а не marks(): getAllByText внутри marks() бросает, когда
    // не нашёл ни одного узла, — а здесь ноль и есть ожидаемый результат.
