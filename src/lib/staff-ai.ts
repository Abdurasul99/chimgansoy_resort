...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: "lookup_facts",
      description:
        "Подробности, которых нет в кратком брифинге: полный тариф бассейна с бунгало и полотенцами (pool), меню ресторана и правила своей еды (menu), дорога и координаты (directions), отмена, перенос, предоплата, депозит, животные (policy), аренда мангала и казана, дрова, уголь, парковка (extras), подробный состав шале и глэмпинга и действующие акции «2+1» и «Всё включено» со сроками (rooms). Вызывай ВМЕСТО того, чтобы отвечать «уточните у администратора».",
      parameters: {
        type: "object",
        properties: {
          topic: {
...
function systemPrompt(live: LivePricing = resolvePricing()): string {
  return [
    ...
    // Было «Валюта — UZS, суммы пиши с разделителями (1 200 000 UZS)» — и бот
    // послушно писал русскому гостю «100 000 UZS». На сайте то же правило давно
    // сформулировано по-человечески; здесь оно отстало.
    "ВАЛЮТА: суммы цифрами с пробелом между тысячами, а название валюты — на языке ответа:",
    "  по-русски «сум» (300 000 сум), по-узбекски «so'm» (300 000 so'm), по-английски «UZS» (300 000 UZS).",
    "  Никогда не пиши «UZS» или «sum» в русском ответе. Разряды считай внимательно, нули не добавляй.",
    "",
    ...
    "4. Рядом с ценой указывай тип и дату с днём недели: «сб ДД.ММ — Глэмпинг <цена из инструмента> сум».",
