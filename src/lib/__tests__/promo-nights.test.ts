...
describe("что показать гостю под датами", () => {
  ...
  it("две ночи со среды: третья ночь уводит выезд в субботу — молчим", () => {
    // Ср→Пт это две ночи и акция на них не распространяется, а продление до
    // субботы её ломает. Предлагать нечего.
    expect(promoHint("2026-09-09", "2026-09-11", СЕГОДНЯ)).toBeNull();
  });

  it("одна ночь или неделя — подсказки нет", () => {
    expect(promoHint("2026-09-07", "2026-09-08", СЕГОДНЯ)).toBeNull();
