...
describe("что показывает блок акций", () => {
  it("на витрине остаётся только «2+1»", () => {
    // Оператор 18.09.2026 убрал две карточки: питание уже названо внутри
    // «2+1», а выходной день администратор подтверждает при брони.
    const shown = promotions.filter((p) => !p.hiddenOnSite).map((p) => p.slug);
    expect(shown).toEqual(["2plus1"]);
  });

  it("скрытые условия остаются в данных, а не удаляются", () => {
    // Консьерж отвечает о них по брифингу. Удалить запись значило бы, что он
    // начнёт отрицать то, что отель на самом деле даёт.
    const hidden = promotions.filter((p) => p.hiddenOnSite).map((p) => p.slug);
    expect(hidden).toEqual(["all-inclusive", "slow-weekend"]);
  });
});
