...
describe("подсказка у границы срока", () => {
  ...
  it("заезд после последнего дня заезда — тоже срок, а не список схем", () => {
    // 29–30.09 акция формально действует, но ни одна схема с этих дат не
    // уложится: одна ночь 29→30 и среда 30.09 → пятница должны назвать срок.
    const late = { kind: "too-late", lastCheckin: "2026-09-28", until: "2026-09-30" };
    expect(promoHint("2026-09-29", "2026-09-30", "2026-09-24")).toEqual(late);
    expect(promoHint("2026-09-30", "2026-10-02", "2026-09-29")).toEqual(late);
  });

  it("срок и последний день заезда в условиях совпадают с расчётом", () => {
    // Строка «Последний заезд — 28 сентября» вписана текстом: промо-данные не
    // могут импортировать расчёт, он сам их читает. Этот тест не даст им
    // разойтись, если срок акции поменяют.
    const promo = promotions.find((p) => p.slug === "2plus1")!;
    const [, m, d] = promoLastCheckin().split("-").map(Number);
    expect(m).toBe(9);
    expect(promo.terms.ru).toContain(`Последний заезд — ${d} сентября`);
    expect(promo.terms.uz).toContain(`Oxirgi kirish kuni — ${d}-sentabr`);
    expect(promo.terms.en).toContain(`Last arrival: ${d} September`);
  });
