...
describe("когда переключаться на следующего", () => {
  it("400, 401, 403, 408, 413, 429 и пятисотые уводят дальше по цепочке", () => {
    // 413 — «Request too large» от Groq: брифинг вместе с зарезервированным
    // ответом не влез в остаток бесплатной минуты. Именно этот код бот показал
    // администратору на вопросе «сколько стоит тюбинг», и, не будучи в списке,
    // он гасил всю цепочку из девяти адресатов на первом же ответе.
    for (const code of [400, 401, 403, 408, 413, 429, 500, 502, 503]) {
      expect(shouldFallThrough(code), String(code)).toBe(true);
    }
  });

  it("успех и «не найдено» дальше не уводят", () => {
