...
describe("когда переключаться на следующего", () => {
  it("429, 401, 403, 400 и пятисотые уводят дальше по цепочке", () => {
    // 400 в этом списке с 2026-08-11: провайдеры расходятся в мелочах, и один
    // отказ не должен гасить оставшихся восьмерых.
    for (const code of [400, 429, 401, 403, 500, 502, 503]) {
