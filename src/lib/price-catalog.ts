  ...
  out.push(
    {
      ...
      label: `Дополнительное место — гость от ${extraGuestPricing.chargedFromAge} лет`,
      hint: `за ночь, одинаково в глэмпинге и шале, для взрослых и детей; дети до ${extraGuestPricing.freeThroughAge} лет включительно бесплатно`,
      value: extraGuestPricing.adult,
    },
    // Строки «Ребёнок 4–12» здесь больше нет: с 2026-08-10 ставка одна на любой
    // возраст от четырёх лет. Поле extraGuest.child осталось в модели данных,
    // чтобы не ломать сохранённый патч, но в админке его быть не должно —
    // строка, которую никто не читает, это приглашение вписать туда число и
    // потом полдня искать, почему оно нигде не появилось.
    {
