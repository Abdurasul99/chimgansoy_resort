...
export async function readAtLeast(minRev: number): Promise<OverrideData> {
  ...
  /**
   * Хранилище так и не догнало за четыре секунды.
   *
   * Отказывать нельзя: замерено на проде — задержка регулярно длиннее, и отказ
   * превращался в «не сохраняется вообще ничего». Пишем поверх того, что есть,
   * и оставляем след в логах.
   *
   * Это единственное место, где правка ещё может потеряться, и лечится оно не
   * ожиданием, а тем, чтобы вовсе не перечитывать документ перед записью: экран
   * и так держит всё нужное состояние и может прислать его целиком.
   */
