...
export async function POST(req: NextRequest) {
  ...
  try {
     ...
     */
    if (!reply) {
      console.warn(`[chat] ${target.label} вернул пустой ответ — уходим на платный запас`);
      /**
       * Повтор идёт как "hard", а это не про сложность вопроса.
       *
       * Цепочка для "hard" начинается мимо Groq: бесплатная модель уже
       * показала, что этот вопрос ей не по бюджету — просить её ещё раз
       * значит потратить квоту и получить ту же пустоту. Платный шлюз
       * отвечает моделью без рассуждения вслух, и бюджет там 2000, а не 900.
       *
       * Платный запас на то и подключён: он должен включаться в тот момент,
       * когда бесплатный не справился, а не оставаться неиспользованным,
       * пока гость читает «Не получилось ответить».
       */
